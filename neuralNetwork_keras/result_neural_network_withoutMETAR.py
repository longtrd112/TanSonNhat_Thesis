...
import csv
import os
from neural_network_keras_withoutMETAR import get_data, CreateNeuralNetworkModel
...
if file_name == 'final_data.csv':
    with open('results_withoutMETAR/result_output_neural_network.txt', 'a') as f:
    ...
    with open('results_withoutMETAR/error_history_neural_network.csv', 'a', newline='') as f:
...
# 3 points
    ...
    with open('results_withoutMETAR/result_output_neural_network_3points.txt', 'a') as f:
        print('Average mean absolute error: ', sum(MAE_history) / len(MAE_history), file=f)
        print('Average root mean squared error: ', sum(RMSE_history) / len(RMSE_history), file=f)
        print('Average mean absolute percentage error: ', sum(MAPE_history) / len(MAPE_history), file=f)

    if not os.path.isfile('results_withoutMETAR/error_history_neural_network_3points.csv'):
        with open('results_withoutMETAR/error_history_neural_network_3points.csv', 'w', newline='') as csv_file:
            writer = csv.writer(csv_file)
            writer.writerow(history_dict.keys())

    with open('results_withoutMETAR/error_history_neural_network_3points.csv', 'a', newline='') as f:
        writer = csv.DictWriter(f, fieldnames=history_dict.keys())
        for values in zip(*history_dict.values()):
            row = dict(zip(history_dict.keys(), values))
            writer.writerow(row)
