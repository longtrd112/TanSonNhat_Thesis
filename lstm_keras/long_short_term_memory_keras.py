...
for i in range(5, 10):
    ...
    early_stopping = tf.keras.callbacks.EarlyStopping(monitor='val_loss', patience=5, min_delta=10, verbose=0)
    model.compile(loss=tf.keras.losses.mean_absolute_error, optimizer=optimizer,
                  metrics=[tf.keras.metrics.MeanAbsoluteError(), tf.keras.metrics.RootMeanSquaredError()])
    # Train the model
    result = model.fit(train_x, train_y, epochs=30, batch_size=4096, validation_data=(val_x, val_y), verbose=1,
